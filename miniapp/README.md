...
**Staging / 合法域名（M1-5）**：当前仓库 **`.env.staging`** 已填团队 staging 根 **`https://api-staging.akamelody.online`**；个人覆盖可建 **`.env.staging.local`**（已 gitignore）。微信公众平台 **request 合法域名** 须与此 **同根**（仅 HTTPS 根，无路径）。执行清单见 **`plan/miniapp/dev/M1-5-合法域名与staging联调.md`**。
